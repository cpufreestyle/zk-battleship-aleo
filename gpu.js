...
/** 初始化 WebGPU 设备 + compute pipeline + 预创建 buffer */
async function init() {
  ...
  try {
    ...
    _storageBuf = _device.createBuffer({
      size: 16,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC,
    });
    _readBuf = _device.createBuffer({
      size: 16,
      usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST,
    });

    console.log("[GPU] WebGPU compute pipeline ready (buffers pre-allocated)");
    return true;
  } catch (e) {
...
/**
 * 在 GPU 上执行 bitwise AND 运算（复用预创建的 buffer）
 * @returns { result, isHit, popcount, isVictory, gpuMs } 或 null
 */
async function computeAnd(ships, mask) {
  if (!_device || !_pipeline || !_uniformBuf || !_storageBuf || !_readBuf) return null;

  let mapped = false;
  try {
    const t0 = performance.now();

    // 写入输入数据（复用 uniform buffer）
    ...
    _device.queue.writeBuffer(_uniformBuf, 0, inputData);
    ...
    pass.setBindGroup(0, _device.createBindGroup({
      ...
      entries: [
        ...
        { binding: 1, resource: { buffer: _storageBuf } },
      ],
    }));
    ...
    encoder.copyBufferToBuffer(_storageBuf, 0, _readBuf, 0, 16);
    _device.queue.submit([encoder.finish()]);

    // read back
    await _readBuf.mapAsync(GPUMapMode.READ);
    mapped = true;
    const resultData = new Uint32Array(_readBuf.getMappedRange());
    ...
    // 确保无论如何都 unmap，buffer 可以被下次复用
    if (mapped) {
      try { _readBuf.unmap(); } catch (_) {}
    }
  }
}
...
/** 销毁所有资源 */
function destroy() {
  if (_uniformBuf) { _uniformBuf.destroy(); _uniformBuf = null; }
  if (_storageBuf) { _storageBuf.destroy(); _storageBuf = null; }
  if (_readBuf) { _readBuf.destroy(); _readBuf = null; }
  _device = null;
  _pipeline = null;
}
